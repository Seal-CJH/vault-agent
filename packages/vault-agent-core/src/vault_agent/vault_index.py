...
class VaultIndex:
    ...
    def _connect(self) -> sqlite3.Connection:
        ...
        connection.execute("CREATE VIRTUAL TABLE IF NOT EXISTS documents USING fts5(path UNINDEXED, title, content, tags, aliases)")
        return connection
    ...
    def rebuild(self) -> int:
        ...
        with self._connect() as connection:
            ...
            connection.executemany(
                "INSERT INTO documents(path, title, content, tags, aliases) VALUES (?, ?, ?, ?, ?)",
                [(d.path, d.title, d.content, " ".join(d.tags), " ".join(d.aliases)) for d in documents],
            )
    ...
    def search(self, query: str, limit: int = 8) -> list[VaultDocument]:
        ...
        with self._connect() as connection:
            rows = connection.execute(
                "SELECT path, title, content, tags, aliases FROM documents WHERE documents MATCH ? ORDER BY rank LIMIT ?",
                (fts_query, limit),
            ).fetchall()
        return [VaultDocument(row[0], row[1], row[2], row[3].split(), row[4].split()) for row in rows]
    ...
    def _read(self, path: Path) -> VaultDocument:
        ...
        return VaultDocument(relative, title, content, tags, aliases)
